...
const DopInfo = ({ infoArray, top, right, typeProduct }) => {
	const [ info, setInfo ] = React.useState([])
	const [showDopInfo, setShowDopInfo ] = React.useState(false)
	...
	React.useEffect(() => {
		if(infoArray){
			...
			const filteredKeys = Object.keys(rest); // получаем список ключей объекта rest
			const filteredValues = filteredKeys.map(key => rest[key]); // создаем массив значений оставшихся полей
