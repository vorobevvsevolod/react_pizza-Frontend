...
const DopProduct: React.FC<IDopProduct> = (props) => {
    const [addInBasket, setAddBasket] = React.useState<Boolean>(false);
	const dispatch = useAppDispatch();
	const onClick = () => {
		setAddBasket(!addInBasket)
		if(!addInBasket){
			dispatch(addDopProduct(props))
		} else dispatch(deleteDopProduct(props.id))

	}
	
	React.useEffect(() =>{
		if(addInBasket){
			dispatch(updatePriceDopProduct({
                id: props.id,
                price: props.price
            }))
		}
	...
	return (
		...
		>
			...
			<img  src={`${process.env.REACT_APP_API_SERVER}/${props.img_url}`} alt=""/>
